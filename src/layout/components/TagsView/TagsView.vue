  ...
  () => {
    ...
    nextTick(() => {
      scrollbarRef.value?.setScrollLeft(
        document.querySelector('.tags-view-wrapper .active')?.offsetLeft
      )
    })
...
const isActive = route => {
  return route.path === $route.path.replace('/redirect', '')
}
const isAffix = tag => {
...
const closeSelectedTag = view => {
  tagsViewStore.delView(view).then(({ visitedViews }) => {
    ...
    if (view.meta?.closeTabRmCache !== false) {
